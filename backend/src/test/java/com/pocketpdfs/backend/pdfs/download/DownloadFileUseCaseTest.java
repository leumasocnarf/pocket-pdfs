...
class DownloadFileUseCaseTest {
    ...
    @Test
    void downloadUrlShouldReturnFileResponseWhenFileExists() {
        ...
        when(storageService.generatePresignedUrl(eq("stub/test.pdf"), any(Duration.class)))
                .thenReturn("http://localhost/stub/test.pdf?expires=300s");

        UrlResponse response = downloadFileUseCase.downloadFile(id);

        assertNotNull(response);
        assertEquals(id, response.id());
        assertEquals("test.pdf", response.filename());
        assertEquals("http://localhost/stub/test.pdf?expires=300s", response.url());
    }

    @Test
    void downloadFileShouldThrowResourceNotFoundExceptionWhenFileDoesNotExist() {
        UUID id = UUID.randomUUID();
        when(repository.findById(id)).thenReturn(Optional.empty());

        assertThrows(ResourceNotFoundException.class, () -> downloadFileUseCase.downloadFile(id));
