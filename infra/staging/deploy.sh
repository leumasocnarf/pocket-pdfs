...
APP_DIR="/opt/pocket-pdfs/staging"
...
until ssh -i $KEY_PATH \
  ...
  -o BatchMode=yes \
  $EC2_USER@$EC2_HOST "test -d /opt/pocket-pdfs/staging" 2>/dev/null; do
...
echo "CORS_ALLOWED_ORIGINS=http://$EC2_HOST:3001" >> $TEMP_ENV
...
echo "==> [6/6] Done! Staging is live at http://$EC2_HOST:3001"